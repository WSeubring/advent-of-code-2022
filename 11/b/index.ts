...
class Monkey {
  ...
  private test(value: bigint) {
    return value % BigInt(this.testValue) === BigInt(0);
  }

  private throwItem(itemWorryLevel: bigint) {
  ...
  public receiveItem(item: bigint) {
...
const parseOperation = (line: string) => {
  const operationFunction = (val: bigint) => {
