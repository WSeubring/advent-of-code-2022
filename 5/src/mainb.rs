            ...
            if let Ok(line) = line {
                if line.is_empty() {
                    ...
                    for _ in 0..movement_command.amount {
                        ...
                        //Naive solution (we should move a splice of the vector instead)
                        crates_to_move.push(crate_name);
                    }

                    // pop items to keep order to the stack (mimic to move at once)
                    while let Some(crate_name) = crates_to_move.pop() {
